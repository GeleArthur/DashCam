<template>
	<div class="scoreboard">
		<div class="scoreboard_wrapper">
			<div class="scoreboard_name scoreboard_name--blue">
				{{ $store.getters.blueTeamName }}
			</div>
			<div class="scoreboard_score scoreboard_score--blue">
				{{ blueTeamScore
				}}<span v-if="matchInfo.matchtype === matchType.Payload">%</span>
			</div>
			<div
				class="scoreboard_mode scoreboard_mode--blue"
				v-if="matchInfo.matchtype"
			>
				<div class="mode mode--blue" :class="matchTypeClass">
					<div
						class="bars"
						v-if="matchInfo.matchtype === matchType.ControlPoint"
					>
						<bar
							:index="teams.blue"
							:compare="'='"
					...
					<div
						class="bars"
						v-else-if="matchInfo.matchtype === matchType.Domination"
					>
						<bar
							:text="'A'"
							:index="teams.blue"
					...
					<div
						class="bars"
						v-else-if="matchInfo.matchtype === matchType.Payload"
					>
						<bar
							v-for="index in 3"
							:index="index"
			...
			<div class="scoreboard_time" v-if="matchInfo.matchtype">
				<div
					v-if="
						matchInfo.matchtype === matchType.Domination &&
						matchInfo.domination.teamCountDown != teams.none
					"
				>
			...
			<div class="scoreboard_name scoreboard_name--red">
...
.scoreboard_wrapper {
	background: transparent url("@/assets/dln-ui-scoreboard.png") no-repeat scroll
		0 0;
	color: #fff;
	display: grid;
	grid-column-start: 2;
...
export default defineComponent({
	...
	computed: mapState({
		players() {
			return this.$store.state.PlayerData.filter(
				(e) => e.team == this.teamID
			);
		},
		redTeamScore() {
			return this.$store.state.matchInfo.redScore
				? this.$store.state.matchInfo.redScore
				: 0;
		},
		blueTeamScore() {
			return this.$store.state.matchInfo.blueScore
				? this.$store.state.matchInfo.blueScore
				: 0;
		},
		teams() {
			return teams;
		...
		matchTypeClass() {
			var mode =
				matchType[this.$store.state.matchInfo.matchtype] !== undefined
					? matchType[this.$store.state.matchInfo.matchtype].toLowerCase()
					: false;
